...
import { generalBtn, watchDateToTimestamp, downloadFile } from '@/utils'
...
export default {
  ...
  data() {
    return {
      params: {
        ...
        staffName: '',
        status: 1,
        time: new Date(new Date().getFullYear() + '-' + (new Date().getMonth() + 1) + '' + '-01 00:00:00').getTime()
      },
      ...
      table: {
        columns: [{
          ...
          title: '可提现佣金',
          key: 'canCashCommission',
          render: (h, { row, column, index }) => {
            return h('span', {
              class: 'c-danger'
            }, this.fmt.currency(row.canCashCommission))
          }
        }, {
          title: '操作',
          render: (h, { row, column, index }) => {
  ...
  created() {
    this.$watch('month', watchDateToTimestamp(() => this.params, 'time'))
  },
  methods: {
    getApi() {
    ...
    exportFile() {
      userCommissionStatApi.exportFile(this.params).then(data => {
        downloadFile(data, '可发放佣金.xls')
      })
    }
  }
}
