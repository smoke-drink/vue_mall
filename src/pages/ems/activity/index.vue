...
export default {
    ...
    return {
      ...
      table: {
        columns: [{
          ...
          key: 'tuanPrice',
          render: (h, { row, column, index }) => {
            return h('span', this.fmt.currency(row.tuanPrice))
          }
        }, {
          title: '单卖价',
          key: 'singlePrice',
          render: (h, { row, column, index }) => {
            return h('span', this.fmt.currency(row.singlePrice))
          }
        }, {
          title: '开始时间',
          key: 'startTime',
          render: (h, { row, column, index }) => {
            return h('span', this.fmt.date(row.startTime))
          }
        }, {
          title: '结束时间',
          key: 'endTime',
          render: (h, { row, column, index }) => {
            return h('span', this.fmt.date(row.endTime))
          }
        }, {
          title: '拼团有效期',
          key: 'limitTime'
  ...
  methods: {
    ...
    updateStatus(id, status, label) {
      activityApi.updateStatus(id, status).then(() => {
