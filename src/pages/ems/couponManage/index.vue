<template>
  <div>
    ...
    <vma-panel v-model="form.show"
               :title="form.entity.id ? '编辑优惠券' : '添加优惠券'">
...
export default {
    ...
    return {
      ...
      table: {
        columns: [{
          ...
          key: 'seckill_price',
          render: (h, { row, column, index }) => {
            return h('span', this.fmt.currency(row.seckill_price))
          }
        }, {
          title: '开始时间',
          key: 'seckill_start',
          render: (h, { row, column, index }) => {
            return h('span', this.fmt.date(row.seckill_start))
          }
        }, {
          title: '结束时间',
          key: 'seckill_over',
          render: (h, { row, column, index }) => {
            return h('span', this.fmt.date(row.seckill_over))
          }
        }, {
          title: '销量',
          key: 'seckill_sales_volume'
        }, {
          title: '操作',
          render: (h, { row, column, index }) => {
            let opts = []
            ...
            if (row.status === 1) {
              opts.push(
                generalBtn(h, '下架', () => {
                  this.updateStatus(row.id, 4, '下架')
                })
              )
            }
            // 已结束且结束时间>当前时间 可上架
            if (row.status === 4 && Date.now() < row.endTime) {
              opts.push(
                generalBtn(h, '上架', () => {
  ...
  methods: {
    ...
    updateStatus(id, status, label) {
      spikeMangeApi.updateStatus({id: id}).then(() => {
